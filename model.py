...
def process_video_part(video_part_path: str, frames_directory: str, start_time: float, end_time: float) -> str:
    extract_frames(video_part_path, frames_directory, frame_interval=2)
    ...
    audio_path = os.path.join(frames_directory, "audio.wav")
...
def process_video(video_path: str, num_parts: int = 3) -> str:
    ...
    summaries = [process_video_part(part[0], frames_directory, part[1], part[2]) for part in video_parts]
    combined_summary = "\n\n".join(summaries)
